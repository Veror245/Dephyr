...
async def apply(body: RemediationRequest, request: Request):
    ...
    async def worker(job_id):
        root = await clone(body.repo, body.base_branch)
        ...
        try:
            ...
            pr = await GitHub(http).create_pr(body.repo, body.title, branch, body.base_branch, body.description)
    ...
    return await runtime.submit('remediation', body.repo, body.cve_id, worker, git_job=True)

@router.post('/followup', status_code=202)
async def followup(body: FollowupRequest):
    require_repo(body.repo)
    async def worker(job_id):
        root = await clone(body.repo, body.branch)
        try:
            ...
            shutil.rmtree(root, ignore_errors=True)
    return await runtime.submit('followup', body.repo, None, worker, git_job=True)

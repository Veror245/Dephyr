...
async def git(*args, cwd: Path | None = None, timeout=90):
    env = os.environ.copy()
    # Prevent interactive prompts, but keep the user's normal Git configuration
    # and credential helpers available. This is important on Windows where Git
    # Credential Manager may be what makes a private-repo clone succeed.
    ...
    if proc.returncode:
        # Keep the API response generic, but log the actual Git error so FastAPI
        # failures can be diagnosed from the server terminal.
        err = stderr.decode(errors='replace').strip()
        out = stdout.decode(errors='replace').strip()
        print(f'[Git] command failed: git {args[0]}', flush=True)
        if err:
            print(f'[Git] stderr: {err}', flush=True)
        if out:
            print(f'[Git] stdout: {out}', flush=True)
